...
@section('content')
    ...
    {{-- ── Form --}}
    <div class="bg-white rounded-2xl border border-zinc-200 overflow-hidden max-w-2xl mx-auto">
        ...
        <div class="p-6 sm:p-8 space-y-5">
            ...
            <div>
                ...
                <input type="text" name="nama_kategori" value="{{ old('nama_kategori', $kategori?->nama_kategori) }}"
                    placeholder="Contoh: Ayam, Seafood, Sayuran" autofocus
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 mb-1.5">
                    Deskripsi <span class="text-zinc-400 font-normal"></span>
                </label>
                <textarea name="deskripsi" rows="4" placeholder="Produk berbahan dasar ayam beku..."
                    class="w-full px-3.5 py-2.5 text-sm border border-zinc-200 rounded-lg outline-none
                                 focus:ring-2 focus:ring-brand-500 focus:border-brand-500 resize-none">{{ old('deskripsi', $kategori?->deskripsi) }}</textarea>
            </div>
        ...
        <div class="px-6 py-4 bg-zinc-50 border-t border-zinc-100 flex items-center justify-end gap-3">
            <a href="{{ route('kategori.index') }}"
                class="px-4 py-2.5 text-sm font-medium text-zinc-600 hover:text-zinc-900 bg-zinc-100 hover:bg-zinc-200 rounded-lg transition-colors">
                Batal
            </a>
            <button type="submit"
                class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold
                           text-white bg-brand-600 hover:bg-brand-700 rounded-lg shadow-sm transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
